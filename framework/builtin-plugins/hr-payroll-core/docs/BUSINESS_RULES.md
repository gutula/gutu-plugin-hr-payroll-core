# HR & Payroll Core Business Rules

## Invariants

- The plugin remains authoritative only for the data declared in `hr-payroll-core` and its owned resource set.
...
- ERP parity references are tracked against: `HR`, `Payroll`, `Projects`, `Manufacturing`.

## Lifecycle notes

- This plugin currently exports 3 action(s), 3 resource(s), 2 job definition(s), and 1 workflow definition(s).
- The domain catalog currently tracks 7 owned entity surface(s), 5 report surface(s), and 4 exception queue(s).
