...
describe("hr-payroll-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "hr-payroll-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "hr-payroll-core:demo",
      title: "HR & Payroll Core Demo",
      ...
      correlationId: "hr-payroll-core:corr",
      processId: "hr-payroll-lifecycle:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "hr-payroll-core:demo",
      exceptionId: "hr-payroll-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "hr-payroll-core:demo",
      amendedRecordId: "hr-payroll-core:demo:amended",
      expectedRevisionNo: 5,
      title: "HR & Payroll Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("hr-payroll-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "hr-payroll-core:demo:amended",
      reversalRecordId: "hr-payroll-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("hr-payroll-core:demo:amended:reversal");
